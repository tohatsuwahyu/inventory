...
btnOpen.onclick = async ()=>{
  ...
  // ベースURLは現在ページのディレクトリを既定値に
  if (!baseEl.value) {
    baseEl.value = new URL('./', location.href).href.replace(/index\.html?$/,'');
  }
};
...
document.getElementById('btn-qr-gen').onclick = async ()=>{
  ...
  let data;
  try { data = await fetchAll(); } catch(e){ alert('シート読み込み失敗: '+e.message); return; }
  ...
  for (const it of list){
    ...
    await new Promise((resolve,reject)=> QRCode.toCanvas(canvas, url, { width:size, margin }, (err)=> err?reject(err):resolve() ));
...
document.getElementById('btn-qr-print').onclick = ()=>{
  ...
  const items = canv.map(cv=>`<div style="page-break-inside:avoid;display:inline-block;margin:10px;text-align:center"><img src="${cv.toDataURL()}" /><div style="font-size:10px">${cv.nextSibling?.textContent||''}</div></div>`).join('');
